...
var APP_MAJOR_VERSION = 2,
    ...
    APP_GOOGLE_ANALYTICS_ID = "UA-74999669-1",
...
var deviceTypeForAdSense = detectDevice(),
    adLoadedDuringInit = !1,
    adNotLoadedReason = "(default)";
...
} else if (!window) (adLoadedDuringInit = !1), (adNotLoadedReason = "window object not available");
else if (window && null != window.location && null != window.location.search) {
    var urlContainsNew = -1 != window.location.search.indexOf("new"),
        urlContainsState = -1 != window.location.search.indexOf("state");
    if (urlContainsNew) adLoadedDuringInit = !0;
    else if (!urlContainsState) (adLoadedDuringInit = !0), (adNotLoadedReason = 'did not find "state" or "new" keywords in URL. window.location.search: ' + window.location.search);
    else if (window && null != window.document && null != window.document.referrer && window.document.referrer.toLowerCase) {
        var isValidReferrer = -1 != window.document.referrer.toLowerCase().indexOf(".google.com");
        isValidReferrer ? (adLoadedDuringInit = !0) : ((adLoadedDuringInit = !0), (adNotLoadedReason = "Detected an invalid referrer: " + window.document.referrer.toLowerCase()));
    } else (adLoadedDuringInit = !0), (adNotLoadedReason = "Did not detect valid referrer to parse. window.document.referrer: " + (window.document ? window.document.referrer : "no_document"));
} else (adLoadedDuringInit = !0), (adNotLoadedReason = "Did not detect valid window search location to parse. window.location.search: " + (window.location ? window.location.search : "no_location"));
...
function updateUiAfterDomLoadedForUnsupportedBrowser() {
    window.console.log("Showing alternate UI for unsupported browser."),
        logImpression("Unsupported browser: updateUiAfterDomLoad()", "app_load"),
        showEl(document.getElementById("mainScrollableContent"), !1),
        showEl(document.getElementById("unsupportedBrowserContent"), !0);
}
function maybeUpdateUiAForAppLoadFailure(a) {
    appLoadFailureUiShown ||
        (isSupportedBrowser &&
            (window.console.log("Showing alternate UI for app load failure."),
            document &&
                "loading" !== document.readyState &&
                (showEl(document.getElementById("mainScrollableContent"), !1),
                showEl(document.getElementById("appLoadErrorContent"), !0),
                (document.getElementById("appLoadErrorDetails").textContent = "Error details: " + a),
                (appLoadFailureUiShown = !0),
                logImpression("app_load_failure", "app_load", a))));
}
function handleDomLoaded() {
    try {
        ...
        if (
            ((domLoaded = !0),
            console.log("In handle Dom Loaded(). domLoaded: " + domLoaded + "; isSupportedBrowser: " + isSupportedBrowser),
            (!adsenseScriptLoadError && adLoadedDuringInit && isSupportedBrowser) ||
                (showEl(document.getElementById("topBanner"), !1),
                showEl(document.getElementById("adHeading"), !1),
                document.getElementById("mainEditorContainer").classList.add("adjustedHeightForNoAdShown"),
                document.getElementById("mainScrollableContent").classList.add("adjustedHeightForNoAdShown"),
                document.getElementById("unsupportedBrowserContent").classList.add("adjustedHeightForNoAdShown"),
                document.getElementById("appLoadErrorContent").classList.add("adjustedHeightForNoAdShown"),
                document.getElementById("mainBackgroundSpinner").classList.add("adjustedHeightForNoAdShownBackgroundSpinner")),
            !isSupportedBrowser)
        )
            return void updateUiAfterDomLoadedForUnsupportedBrowser();
        ...
        document.getElementById("startPageIconImgDrive").removeAttribute("visibilityHidden"),
            showEl(document.getElementById("startPageIconImgGeneral"), e || isForDirectNew),
            showEl(document.getElementById("startPageIconImgGmail"), d),
            showEl(document.getElementById("startPageIconImgDrive"), c),
            showEl(document.getElementById("startPageGeneralDetails"), e),
            showEl(document.getElementById("startPageGmailDetails"), d),
            showEl(document.getElementById("startPageDriveDetails"), c),
            showEl(document.getElementById("startPageNewDetails"), isForDirectNew),
            (document.getElementById("startPageDriveDetails").style.visibility = ""),
            showEl(document.getElementById("startPagePrompt"), !0),
            (document.getElementById("startPagePrompt").style.visibility = ""),
            MAINAPP && (MAINAPP.initAfterDomLoaded(getFileId()), EDITOR && MAINAPP.initializeEditorAfterDomLoaded());
    } catch (a) {
...
function logImpression(a, b, c, d, e, f) {
    var g = {};
    null != b && ((g.event_category = b), "app_load" === b && null == e),
        null != c && (g.event_label = c),
        null != d && (g.value = +d),
        !0 == f && (g.transport_type = "beacon"),
        !0 == e && (g.non_interaction = !0),
        0 == Object.keys(g).length ? gtag("event", a) : gtag("event", a, g);
}
function clearUrlState() {
    window.history.replaceState({}, "", window.location.pathname);
}
function onAnalyticsScriptLoaded() {
    console.log("analytics_script_loaded"),
        (analyticsLoaded = !0),
        adLoadedDuringInit || logImpression("AD NOT LOADED; adLoadedDuringInit=false.", "app_load", "reason: " + adNotLoadedReason),
...
function onMainAppScriptLoaded() {
    ...
    function onGapiScriptLoadError(a) {
        handleScriptLoadError("gapi_load_error", a, "hasBeenAutoRetried: " + hasGapiScriptBeenAutoRetried),
            hasGapiScriptBeenAutoRetried
                ? isForOpen() && ((isAppLoadFailure = !0), (appLoadFailureMessage = "gapi_load_error: " + getErrorStringFromErrorOrEvent(a)), maybeUpdateUiAForAppLoadFailure(appLoadFailureMessage))
                : ((hasGapiScriptBeenAutoRetried = !0), loadGapiScript());
    }
    function onMainAppScriptLoadError(a) {
        handleScriptLoadError("main_app_script_load_error", "hasBeenAutoRetried: " + hasMainAppScriptBeenAutoRetried),
            hasMainAppScriptBeenAutoRetried
                ? ((isAppLoadFailure = !0), (appLoadFailureMessage = "main_app_script_load_error: " + getErrorStringFromErrorOrEvent(a)), maybeUpdateUiAForAppLoadFailure(appLoadFailureMessage))
                : ((hasMainAppScriptBeenAutoRetried = !0), loadMainAppScript());
    }
    ...
    function onAdSenseScriptLoadError() {
        (adsenseScriptLoadError = !0),
            handleScriptLoadError("adsense_script_load_error", "hasBeenAutoRetried: " + hasAdSenseScriptBeenAutoRetried),
            adsenseScriptLoadErrorHasBeenLogged || (logImpression("ADSENSE SCRIPT LOAD ERROR", "app_load", "source: onAdSenseScriptLoadError"), (adsenseScriptLoadErrorHasBeenLogged = !0));
    }
    ...
    function onAceEditorScriptLoadError(a) {
        handleScriptLoadError("ace_editor_script_load_error", "hasBeenAutoRetried: " + hasAceEditorScriptBeenAutoRetried),
            hasAceEditorScriptBeenAutoRetried
                ? ((isAppLoadFailure = !0), (appLoadFailureMessage = "ace_editor_script_load_error: " + getErrorStringFromErrorOrEvent(a)), maybeUpdateUiAForAppLoadFailure(appLoadFailureMessage))
                : ((hasAceEditorScriptBeenAutoRetried = !0), loadAceEditorScript());
    }
    ...
    function onGapiInit() {
        console.log("*** onGapiInit() CALLBACK"),
            execLater(
                function () {
    ...
    function authorize(a, b, c, d, e, f, g, h) {
        ...
        var j = { client_id: APP_CONFIG.clientId, scope: APP_CONFIG.scopes, response_type: "token id_token" };
    ...
    function updateUiForDevice() {
        try {
            var a = detectDevice();
            ("phone" == a || "device" == a) &&
                (updateButtonTextHelper("openFileFromDriveButton", "Open From Drive"),
                updateButtonTextHelper("createNewTextFileButton", "New File"),
                updateButtonTextHelper("openFileFromComputerButton", "Open From Phone", "smartphone"),
                "device" == a || "phone" == a);
        } catch (a) {
    ...
    function maybeHandleDomLoadedOrRegister() {
        ...
        ("loading");
        document.readyState
            ? (console.log("maybeHandleDomLoadedOrRegister(): DOM not yet ready, registering."),
    ...
    function loadMainAppScript() {
        loadScript("core-2-11-min.js"), bindFn(onMainAppScriptLoaded), bindFn(onMainAppScriptLoadError);
    }
    function loadAnalyticsScript() {
        loadScript("https://www.googletagmanager.com/gtag/js?id=UA-74999669-1", bindFn(onAnalyticsScriptLoaded), bindFn(onAnalyticsScriptLoadError));
    }
    ...
    function loadAceEditorScript() {
        loadScript("https://cdnjs.cloudflare.com/ajax/libs/ace/1.4.13/ace.js", bindFn(onAceEditorScriptLoaded), bindFn(onAceEditorScriptLoadError));
    }
    function loadJsChardetScript() {
        loadScript("https://cdnjs.cloudflare.com/ajax/libs/jschardet/2.1.1/jschardet.min.js", bindFn(onJsChardetScriptLoaded), bindFn(onJsChardetScriptLoadError));
    }
    isSupportedBrowser
        ? (loadGapiScript(), loadMainAppScript(), loadAnalyticsScript(), loadAceEditorScript(), loadJsChardetScript(), true ? loadAdSenseScript() : console.log("Skipping AdSense load."))
        : (logImpression("Unsupported browser: Skipping Script Load.", "app_load"), window.console.log("Skipping loading additional scripts for unsupported browser."));
}
